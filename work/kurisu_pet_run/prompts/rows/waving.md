Create one horizontal animation strip for Codex pet `kurisu`, state `waving`.
...
State action: A restrained acknowledgement rather than a cheerful mascot wave: notice the user, lift one hand only slightly in a concise half-wave, hold a composed neutral mouth with faintly softened eyes, then lower the hand and recover posture. No broad smile or enthusiastic arm swing.
