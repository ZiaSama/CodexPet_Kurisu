Create one horizontal animation strip for Codex pet `kurisu`, state `running`.
...
State action: Focused researcher work loop using a small understated dark terminal/tablet already integrated into the pose: read rapidly with eye movement, type briefly with controlled hands, pause, bring one hand near the chin, make a tiny analytical frown, then resume reading. The prop has no legible UI or text. Do not type continuously or frantically.
